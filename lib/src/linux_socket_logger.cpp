...
void SocketLogger::init(const std::string& ip, const unsigned short& port) {
    if (!ready) {
        // no need to start wsa or anything
...
void SocketLogger::setupListeningSocket() {
    ...
    // which is not exactly straighforward in linux
    int flags = fcntl(listen_socket, F_GETFL, 0);
    if (flags == -1) {
        std::cout << "\tFailed to set flags.\n";
    }
        
    flags |= O_NONBLOCK;
    if (fcntl(listen_socket, F_SETFL, flags) == -1) {
        std::cout << "\tFailed to set listening socket to non-blocking.\n";
    }
}
...
common::socket::ClientWaitResult SocketLogger::waitForClient() {
    ...
    // std::cout << "\tWaiting for a client to connect...\n";
    {
        ...
        // there was also an option to use poll(), but since the while(true) loop
        // is outside of waitForClient(), stick with simpler logic
        client_socket = accept(listen_socket, nullptr, nullptr);
        if (client_socket == INVALID_SOCKET) {
            // check the errno against BOTH eagain AND ewouldblock
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                // no client connection yet, no error though
